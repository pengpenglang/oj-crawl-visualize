...
def assio(teams, name_num):
    ...
    for i in range(NUM):
        teams.append(TEAM(df.iloc[i][0], df.iloc[i][2], df.iloc[i][3], df.iloc[i][4], i))
        name_num[teams[i].name] = i
    return


def read(teams, name_num):
    # Read the data from the excel file
    file = pd.read_excel('../sheets/hdu.xlsx', sheet_name=None)
    # get sheet num
    sheet_num = len(file)
    # 输出第一行
    for i in range(1, sheet_num + 1):
        ...
        for i in range(NUM):  # 队伍当前场次信息存入
            if (df.iloc[i][0] == '#'):
                teams[name_num[df.iloc[i][1]]].infos.append(INFO(None, 0, 0))
            else:
                teams[name_num[df.iloc[i][1]]].infos.append(INFO(int(df.iloc[i][0]), int(df.iloc[i][2]), df.iloc[i][3]))
...
def draw(teams):
    ...
    line_chart.render_to_file('./hdu/hdu_line.svg')
    ...
    bar_chart.render_to_file('./hdu/hdu_bar.svg')
    ...
    pie_chart.render_to_file('./hdu/hdu_pie.svg')
    return
